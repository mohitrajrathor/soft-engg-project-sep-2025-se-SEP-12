    ...
    <main class="flex-1 overflow-y-auto p-6">
      ...
      <section v-show="activePage==='ai-assistant'" class="h-full flex flex-col gap-4">
        ...
        <div class="flex-1 bg-white border border-gray-200 rounded-xl p-4 flex flex-col">
          <div id="chatMessages" class="flex-1 overflow-y-auto p-4 space-y-3">
            <div v-for="(m, i) in messages" :key="i" :class="m.from === 'user' ? 'flex justify-end' : 'flex justify-start'">
          ...
          <div class="p-4 border-t border-gray-200">
            <div class="flex gap-2">
              <input v-model="chatInput" @keyup.enter="sendMessage" placeholder="Ask a question..." class="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600" />
              <button @click="sendMessage" class="px-5 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">Send</button>
            </div>
          </div>
        </div>
      </section>
      ...
      <section v-show="activePage==='study-assistant'" class="space-y-6">
        <div class="bg-white border border-gray-200 rounded-xl p-6">
          ...
          <p class="text-sm text-gray-600">Weekly summary, deadlines, and smart reminders will appear here. (API-ready)</p>
        </div>

        <!-- Example card -->
        <div class="bg-white border border-gray-200 rounded-xl p-6">
          <h4 class="font-medium">Weekly Summary</h4>
          <p class="text-sm text-gray-600 mt-2">This is a placeholder. Hook up the backend to fetch weekly summaries.</p>
        </div>
      </section>
